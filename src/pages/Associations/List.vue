<template>
    <div>
        ...
        <v-dialog v-model="form_dialog" persistent max-width="500px">
            <v-card>
                <v-card-title>
                    <span class="text-h5">Associations Form</span>
                </v-card-title>
                <v-card-text>
                    <v-container>
                        <v-row>
        ...
        <ConfirmDeleteDialog v-model="delete_dialog" @confirm="handleDeleteSubmit">
            <template #title>
                Confirm to delete Association?
            </template>
            <template #content>
                Are you sure you want to delete "{{ actual_model.name }}" association?
            </template>
        </ConfirmDeleteDialog>
    </div>
</template>
...
import { useAssociationsStore } from '@/stores/associations/index'
...
let items: Ref<Array<Association>> = computed(() => {
...
let actual_model: Association = reactive(new Association())
...
let handleFormSubmit = async function () {
    ...
    if (!v$.value.actual_model.name.$error) {
        ...
        if(actual_model.id == null){
            store$.store(new Association(actual_model)).then((response: any) => {
                loading.value = false
            })
        } else {
            store$.update(new Association(actual_model)).then((response: any) => {
                loading.value = false
            })
        }
    }
...
let handleDeleteSubmit = function () {
    ...
    loading.value = true
    store$.delete(new Association(actual_model)).then((response: any) => {
        loading.value = false
    })
}
