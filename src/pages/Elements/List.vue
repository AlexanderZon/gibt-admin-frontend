<template>
    <div>
        ...
        <v-dialog v-model="form_dialog" persistent max-width="500px">
            <v-card>
                <v-card-title>
                    <span class="text-h5">Elements Form</span>
                </v-card-title>
                <v-card-text>
                    <v-container>
                        <v-row>
        ...
        <ConfirmDeleteDialog v-model="delete_dialog" @confirm="handleDeleteSubmit">
            <template #title>
                Confir Delete Element?
            </template>
            <template #content>
                Are you sure you want to delete "{{ actual_model.name }}" element?
            </template>
        </ConfirmDeleteDialog>
    </div>
</template>
...
let items: Ref<Array<Element>> = computed(() => {
...
let actual_model: Element = reactive(new Element())
let setActualModel = function(_element: any){
    v$.value.actual_model.name.$reset()
    actual_model.fill(_element)
}
let showFormDialog = function (_element: any) {
    setActualModel(_element);
...
let handleFormSubmit = async function () {
    ...
    if (!v$.value.actual_model.name.$error) {
        ...
        if(actual_model.id == null){
            store$.store(new Element(actual_model)).then((response: any) => {
                loading.value = false
            })
        } else {
            store$.update(new Element(actual_model)).then((response: any) => {
                loading.value = false
            })
        }
    }
...
let showDeleteDialog = function (_element: any) {
    setActualModel(_element)
...
let handleDeleteSubmit = function () {
    ...
    loading.value = true
    store$.delete(new Element(actual_model)).then((response: any) => {
        loading.value = false
    })
}
